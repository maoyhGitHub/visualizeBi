 ...
 * @param value
 */
function textTreeWidget(nodeCache, name, value) {
  if (name === 'innerBgColor') {
    if (value == null) {
      nodeCache.querySelector('.el-tree').style.backgroundColor = 'rgba(0,0,0,0)'
    } else {
      nodeCache.querySelector('.el-tree').style.backgroundColor = value
    }
  }
  if (name === 'wordColor') {
    nodeCache.querySelector('.el-tree').style.color = value
  }
}

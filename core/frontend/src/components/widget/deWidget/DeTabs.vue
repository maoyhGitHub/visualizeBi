<template>
  ...
  >
    ...
    <dataease-tabs
      ...
      :bind-custom-style="bindCustomStyle"
    ...
    >
      ...
      <el-tab-pane
        ...
        :name="item.name"
      >
        <span slot="label" :style="{top:'30%',position: 'inherit','--quotaColor': quotaColor,'--quotaColor1':quotaColor1}">
          <span v-if="quotaColorFlag" :style="titleStyle(item.name)" :class="['gradient-text']">{{ item.title }}</span>
          <span v-else :style="titleStyle(item.name)">{{ item.title }}</span>
          <el-dropdown
            v-if="dropdownShow"
            slot="label"
            class="de-tab-drop"
...
export default {
  ...
  computed: {
    ...
    fontColor() {
      ...
      } else if (this.element && this.element.style && this.element.style.headFontColor1 && typeof this.element.style.headFontColor1 === 'string') {
        return this.element.style.headFontColor1
      } else {
        return 'none'
      }
    },
    ...
    bindCustomStyle() {
      if (this.element && this.element.style && this.element.style) {
        const bindCustomStyle = {
          activeBackgroundColor: 'rgba(255,255,255,0)',
          titlePanelHeight: '40px',
          activeBackgroundImage: false,
          titlePanelRight: '0px'
        }
        if (this.element.style.activeBackgroundColor && typeof this.element.style.activeBackgroundColor === 'string') {
          bindCustomStyle.activeBackgroundColor = this.element.style.activeBackgroundColor
        }
        if (this.element.style.titlePanelHeight) {
          bindCustomStyle.titlePanelHeight = this.element.style.titlePanelHeight + 'px'
        }
        if (this.element.style.activeBackgroundImage) {
          bindCustomStyle.activeBackgroundImage = this.element.style.activeBackgroundImage
        }
        if (this.element.style.titlePanelRight) {
          bindCustomStyle.titlePanelRight = this.element.style.titlePanelRight + 'px'
        }
        return bindCustomStyle
      } else {
        return 'none'
      }
    },
  ...
  methods: {
    ...
    titleStyle(itemName) {
      if (this.element.style.headFontColor && this.element.style.headFontColor1) {
        this.quotaColorFlag = true;
        this.quotaColor = this.element.style.headFontColor
        this.quotaColor1 = this.element.style.headFontColor1
      } else {
        this.quotaColorFlag = false
      }
...
.gradient-text {
  ...
  background: linear-gradient(to bottom,var(--quotaColor) , var(--quotaColor1));

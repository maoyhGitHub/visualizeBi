...
import { attrsMap, styleAttrs, textSelectWidget, textTreeWidget } from '@/components/widget/deWidget/serviceNameFn.js'
import ElTreeListSelect from '@/components/elTreeListSelect/index.vue'

export default {
  components: { ElTreeListSelect },
  mixins: [customInput],
  props: {
    canvasId: {
      type: String,
  ...
  data() {
    return {
      ...
      imgsty: require('@/components/canvas/assets/bg-kj-1.jpg')
    }
  },
  computed: {
    operator() {
  ...
  watch: {
    ...
    'element.options.attrs.fieldId': function(value, old) {
      ...
      method(param).then(res => {
        ...
        this.$nextTick(() => {
          this.$refs.deSelectTreeList && this.$refs.deSelectTreeList.treeDataUpdateFun(this.data)
        })
      })
  ...
  methods: {
    clearHandler() {
      ...
      this.$refs.deSelectTreeList && this.$refs.deSelectTreeList.resetSelectAll && this.$refs.deSelectTreeList.resetSelectAll()
    },
    ...
    _searchFun(value) {
      // 自行判断 是走后台查询，还是前端过滤
      this.$refs.deSelectTreeList.filterFun(value)
      // 后台查询
      // this.$refs.deSelectTreeList.treeDataUpdateFun(treeData);
    },
    // 自定义render
    _renderFun(h, { node, data, store }) {
      node.imgsty = this.imgsty
      ...
      return (
        ...
        >
          <span >
            <svg-icon icon-class='airplane_1' />
          </span>
          <span>{node.label}</span>
          <span style= 'position: absolute; left: 96%;'> {data.children ? data.children.length : 0} </span>
        </span>
      )
    },
    ...
    changeInputStyle() {
      ...
      this.$nextTick(() => {
        this.handlerInputStyle(this.customStyle)
      })
    },
    handlerInputStyle(newValue) {
      if (!this.$children[0].$refs['de-tree-list']) return
      styleAttrs.forEach(ele => {
        this.textTreeWidget(this.$el, ele, newValue[ele])
      })
      this.onFocus();
    }

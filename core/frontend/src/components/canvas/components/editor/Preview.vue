<template>
  ...
  >
    ...
    <user-view-Open-dialog
      ...
      :open-dialog-url="openParam.openDialogUrl"
      :is-edit="false"
      :jump-info-data="openParam.jumpInfoData"
...
export default {
  components: {UserViewDialog, ComponentWrapper, CanvasOptBar, PDFPreExport, UserViewOpenDialog},
  model: {
    prop: 'show',
    event: 'change'
  },
  ...
  mounted() {
    ...
    bus.$on('pcOpenDialog', this.openShowDialog)
  ...
  methods: {
    ...
    openShowDialog(paramInfo) {
      if (this.canvasId === 'canvas-main') {
        this.openParam = paramInfo

<template>
  ...
  >
    ...
    <plugin-com
      ...
      :obj="{active, chart, trackMenu, searchCount, terminalType: scaleCoefficientType,inScreen:inScreen}"
      :chart="chart"
    ...
    />
    <dev-tool-index-component
      v-else-if="devToolShowFlag"
      :ref="element.propValue.id"
      :obj="{active, chart, trackMenu, searchCount, terminalType: scaleCoefficientType,inScreen:inScreen}"
      :chart="chart"
      :track-menu="trackMenu"
    ...
    <el-dialog
      ...
      top="5vh"
    >
      <span style="position: absolute; right: 200px; top: 15px; }">
        <AddCustomExcel :param="excelParam"/>
      </span>
      <span
        style="position: absolute;right: 70px;top:15px"
      >
        ...
        >
          <svg-icon
            icon-class="ds-excel"
            class="ds-icon-excel"
          />{{ $t('chart.export') }}Excel模板
        </el-button>
      </span>
      <user-custom-view-dialog
        v-if="showCustomDialog"
        ref="userCustomViewDialog"
        :chart="showChartInfo"
        :chart-table="showChartTableInfo"
        :canvas-style-data="canvasStyleData"
        :open-type="showChartInfoType"
    ...
    <!--    弹出页详细-->
    <user-view-Open-dialog
      v-if="showOpenDialogVisible"
      :open-dialog-url="openDialogUrl"
      :is-edit="true"
      :jump-info-data="jumpInfoData"
      @closeDialogSetDialog="showOpenDialogVisible = false"
    />
  </div>
</template>
...
import { viewInfo, updateTempStaticData, getViewTable } from '@/api/link'
...
import UserCustomViewDialog from './UserCustomViewDialog'
import AddCustomExcel from '@/components/canvas/customComponent/AddCustomExcel'
import DevToolIndexComponent from '@/devtool/pluginDev/index.vue'
import UserViewOpenDialog from '@/components/canvas/customComponent/UserViewOpenDialog.vue'
...
export default {
  ...
  components: {
    ...
    DrillPath,
    ChartComponentG2,
    UserCustomViewDialog,
    AddCustomExcel,
    DevToolIndexComponent,
    UserViewOpenDialog
  },
  props: {
    inScreen: {
  ...
  computed: {
    ...
    devToolShowFlag() {
      return this.httpRequest.status && this.chart.type && this.chart.type === 'view-dev-tool' && this.renderComponent() === 'custom'
    },
    panelInfo() {
  ...
  methods: {
    ...
    resizeChart() {
      if (this.chart.type === 'map' || this.chart.type === 'view-dev-tool') {
    ...
    openChartDetailsDialog(params) {
      ...
      if (!this.inScreen) {
        ...
        if (this.showChartInfoType === 'customData') {
          getViewTable(this.chart.tableId).then(res => {
            if (res.data && res.data.type === 'excel') {
      ...
      if (jumpInfo) {
        ...
        if (jumpInfo.linkType === 'inner') {
          if (jumpInfo.targetPanelId) {
            param.toTargetPanelId = jumpInfo.targetPanelId
            localStorage.setItem('buttonJumpParam', JSON.stringify(param))
            ...
            if (this.publicLinkStatus) {
              ...
              if (jumpInfo.jumpType === '_open') {
                this.openDialogUrl = url
                this.jumpInfoData = jumpInfo
                if (!this.inScreen) {
                  const openParam  ={
                    jumpInfoData:jumpInfo,
                    openDialogUrl: url
                  }
                  bus.$emit('pcOpenDialog', openParam)
                }else {
    ...
    exportCustomExcel() {
      this.$refs['userCustomViewDialog'].exportExcel()
    },
    closeDel() {
      this.showCustomDialog = false
    },
    confirmRelease() {
      const { fullData } = this.$refs.userCustomViewDialog.$refs.tableEditNormalRef.getPlxTableData()
      ...
      const excelData = fullData.map(item => excelHeaderKeys.map(i => item[i]))
      const request = {
        status: 1,
        viewId: this.chart.id,
        tableId: this.chart.tableId,
        newJsonDetails: JSON.stringify(fullData),
        excelNewData: JSON.stringify(excelData)
      }
      updateTempStaticData(request).then(res => {
        this.closeDel()
        this.$message({
          message: '发布成功，待审批通过后，请重新刷新页面',
          type: 'success'
        })
      })
    }

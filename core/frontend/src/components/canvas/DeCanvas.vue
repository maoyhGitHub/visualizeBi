  ...
  >
    ...
    >
      ...
      <div v-if="showFlag" style="height: 452px;overflow-y: auto">
        <el-table-column
          v-for="(item,index) in urlList" :key="index" class="img">
          <el-popover
            :ref="index"
...
export default {
  ...
  methods: {
    ...
    submitPicture(pictureViewVisible){
      if(this.fileName){
    ...
    handlePicturChange() {
      ...
      uploadPictureResult(this.fileName, (fileUrl) => {
        const fileId = this.fileName.substring(0,this.fileName.lastIndexOf("."));
        const component = {
          ...commonAttr,
          id: fileId,
          component: 'Picture',
    ...
    handleDrop(e) {
      ...
      if (componentInfo.type === 'assist') {
        ...
        if (component.type === 'picture-view') {

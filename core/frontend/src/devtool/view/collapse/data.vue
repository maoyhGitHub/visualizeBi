  ...
  <div>
    ...
    <el-row class="padding-lr">
      <span style="width: 80px;text-align: right;">
       <span>分类/{{ $t('chart.dimension') }}</span>
      </span>
      <draggable
        v-model="categoryXaxis"
        group="drag"
        animation="300"
        :move="onMove"
      ...
      >
        <transition-group class="draggable-group">
          <dimension-item
            v-for="(item,index) in categoryXaxis"
            :key="item.id"
            :param="param"
            :index="index"
            :item="item"
      ...
      <div
        v-if="!categoryXaxis || categoryXaxis.length === 0"
    ...
    >
      <span class="data-area-label">
        <span>内容/ {{ $t('chart.dimension') }}</span>
      </span>
      <draggable
        v-model="contentXaxis"
        group="drag"
        animation="300"
        :move="onMove"
        class="drag-block-style"
      ...
      >
        <transition-group class="draggable-group">
          <dimension-item
            v-for="(item,index) in contentXaxis"
            :key="item.id"
            :param="param"
            :index="index"
            :item="item"
      ...
      <div
        v-if="!contentXaxis || contentXaxis.length === 0"
        class="drag-placeholder-style"
      >
        <span class="drag-placeholder-style-span">{{ $t('chart.placeholder_field') }}</span>
      </div>
...
export default {
  ...
  created() {
    ...
    this.view.xaxis.forEach(ietm=>{
      switch (ietm.flag) {
        case 'categoryXaxis':
          this.categoryXaxis = [ietm]
          break;
        case 'contentXaxis':
          this.contentXaxis = [ietm]
          break;
      }
    })
  },
  methods: {
    ...
    addXaxis(e) {
      this.dragCheckType(this.categoryXaxis, 'd')
      this.dragMoveDuplicate(this.categoryXaxis, e)
      this.categoryXaxis[0].flag = 'categoryXaxis';
      if (this.categoryXaxis.length > 1) {
         this.categoryXaxis = [this.categoryXaxis[0]]
      }
      this.calcData(true)
    },
    ...
    calcData(cache) {
      this.view.xaxis = [...this.categoryXaxis,...this.contentXaxis];
    ...
    dimensionItemRemove(item) {
      if (item.removeType === 'dimension') {
        this.categoryXaxis.splice(item.index, 1)
    ...
    xaxisRemove(item) {
      if (item.removeType === 'dimension') {
        this.contentXaxis.splice(item.index, 1)
      } else if (item.removeType === 'dimensionExt') {
        this.view.xaxisExt.splice(item.index, 1)
      }
      this.calcData(true)
    ...
    //经度
    addContentXaxis(e){
      this.dragCheckType(this.contentXaxis, 'd')
      this.dragMoveDuplicate(this.contentXaxis, e)
      this.contentXaxis[0].flag = 'contentXaxis';
      if (this.contentXaxis.length > 1) {
        this.contentXaxis = [this.contentXaxis[0]]
      }
      this.calcData(true)
    },
  }

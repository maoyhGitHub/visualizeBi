  ...
  <div>
    ...
    <el-row class="padding-lr">
      <span style="width: 80px;text-align: right;">
        <span>省份</span>/<span>{{ $t('chart.dimension') }}</span>
      </span>
      <draggable
        v-model="provinceXaxis"
      ...
      >
        <transition-group class="draggable-group">
          <dimension-item
            v-for="(item,index) in provinceXaxis"
            :key="item.id"
            :param="param"
            :index="index"
            :item="item"
      ...
      <div
        v-if="!provinceXaxis || provinceXaxis.length === 0"
    ...
    >
      ...
      >
        <transition-group class="draggable-group">
          <dimension-item
            v-for="(item,index) in locationXaxis"
            :key="item.id"
            :param="param"
            :index="index"
            :item="item"
            ...
            :quota-data="quotaData"
            @onDimensionItemChange="dimensionItemChange"
            @onDimensionItemRemove="xaxisRemove"
            @editItemFilter="showDimensionEditFilter"
            @onNameEdit="showRename"
          />
        </transition-group>
      ...
      <div
        v-if="!locationXaxis || locationXaxis.length === 0"
        class="drag-placeholder-style"
      >
        <span class="drag-placeholder-style-span">{{ $t('chart.placeholder_field') }}</span>
      </div>
    ...
    >
      ...
      >
        <transition-group class="draggable-group">
          <dimension-item
            v-for="(item,index) in locationYaxis"
            :key="item.id"
            :param="param"
            :index="index"
            :item="item"
            ...
            :quota-data="quotaData"
            @onDimensionItemChange="dimensionItemChange"
            @onDimensionItemRemove="locationYaxisRemove"
            @editItemFilter="showDimensionEditFilter"
            @onNameEdit="showRename"
          />
        </transition-group>
      ...
      <div
        v-if="!locationYaxis || locationYaxis.length === 0"
        class="drag-placeholder-style"
      >
        <span class="drag-placeholder-style-span">{{ $t('chart.placeholder_field') }}</span>
      </div>
    ...
    >
      <span class="data-area-label">
        <span>绑定战区/ {{ $t('chart.dimension') }}</span>
      </span>
      <draggable
        v-model="warzone"
        group="drag"
      ...
      >
        <transition-group class="draggable-group">
          <dimension-item
            ...
            :quota-data="quotaData"
            @onDimensionItemChange="dimensionItemChange"
            @onDimensionItemRemove="warzoneRemove"
            @editItemFilter="showDimensionEditFilter"
            @onNameEdit="showRename"
          />
        </transition-group>
    ...
    >
      <span class="data-area-label">
        <span>悬浮提示框/ {{ $t('chart.dimension_or_quota') }}</span>
      <i
      ...
      >
        <transition-group class="draggable-group">
          <dimension-item
            ...
            :quota-data="quotaData"
            @onDimensionItemChange="dimensionItemChange"
            @onDimensionItemRemove="toolTipRemove"
            @editItemFilter="showDimensionEditFilter"
            @onNameEdit="showRename"
          />
        </transition-group>
...
import LocationItem from '@/devtool/components/views/LocationItem.vue'
import { getTooltip } from '@/views/chart/chart/common/common_antv'


export default {
  components: { DimensionItem, QuotaItem, FilterItem, DrillItem , LocationItem},
  props: {
  ...
  created() {
    ...
    this.view.xaxis.forEach(ietm=>{
      switch (ietm.flag) {
        case 'provinceXaxis':
          this.provinceXaxis = [ietm]
          break;
        case 'locationXaxis':
          this.locationXaxis = [ietm]
          break;
        case 'locationYaxis':
          this.locationYaxis = [ietm]
          break;
        case 'warzone':
          this.warzone = [ietm]
        case 'toolTip' :
  ...
  methods: {
    ...
    addXaxis(e) {
      this.dragCheckType(this.provinceXaxis, 'd')
      this.dragMoveDuplicate(this.provinceXaxis, e)
      this.provinceXaxis[0].flag = 'provinceXaxis';
      if (this.provinceXaxis.length > 1) {
        this.provinceXaxis = [this.provinceXaxis[0]]
      }
      this.calcData(true)
    },
    ...
    calcData(cache) {
      this.view.xaxis = [...this.provinceXaxis,...this.locationXaxis, ...this.locationYaxis,...this.warzone,...this.toolTip];//经纬度信息
    ...
    dimensionItemRemove(item) {
      if (item.removeType === 'dimension') {
        this.provinceXaxis.splice(item.index, 1)
    ...
    xaxisRemove(item) {
      if (item.removeType === 'dimension') {
        this.locationXaxis.splice(item.index, 1)
      } else if (item.removeType === 'dimensionExt') {
        this.view.xaxisExt.splice(item.index, 1)
      }
      this.calcData(true)
    },
    locationYaxisRemove(item) {
      if (item.removeType === 'dimension') {
        this.locationYaxis.splice(item.index, 1)
      } else if (item.removeType === 'dimensionExt') {
        this.view.xaxisExt.splice(item.index, 1)
      }
      this.calcData(true)
    ...
    warzoneRemove(item) {
      if (item.removeType === 'dimension') {
        this.warzone.splice(item.index, 1)
      } else if (item.removeType === 'dimensionExt') {
        this.view.xaxisExt.splice(item.index, 1)
      }
      this.calcData(true)
    ...
    //经度
    addLocationXaxis(e){
      this.dragCheckType(this.locationXaxis, 'd')
      this.dragMoveDuplicate(this.locationXaxis, e)
      this.locationXaxis[0].flag = 'locationXaxis';
      if (this.locationXaxis.length > 1) {
        this.locationXaxis = [this.locationXaxis[0]]
    ...
    //纬度
    addLocationYaxis(e){
      this.dragCheckType(this.locationYaxis, 'd')
      this.dragMoveDuplicate(this.locationYaxis, e)
      this.locationYaxis[0].flag = 'locationYaxis';
      if (this.locationYaxis.length > 1) {
        this.locationYaxis = [this.locationYaxis[0]]
      }
      this.calcData(true)
    },
    addWarzone(e){
      this.dragCheckType(this.warzone, 'd')
      this.dragMoveDuplicate(this.warzone, e)
      this.warzone[0].flag = 'warzone';
      if (this.warzone.length > 1) {
        this.warzone = [this.warzone[0]]
      }
      this.calcData(true)
    },
    addToolTip(e){
      this.dragMoveDuplicate(this.warzone, e)
      this.toolTip.forEach(ietm=>{
        ietm.flag  = 'toolTip'
      })
      this.calcData(true)
    },
    toolTipRemove(item) {
      if (item.removeType === 'dimension') {
        this.toolTip.splice(item.index, 1)
      } else if (item.removeType === 'dimensionExt') {
        this.view.xaxisExt.splice(item.index, 1)
      }
      this.calcData(true)

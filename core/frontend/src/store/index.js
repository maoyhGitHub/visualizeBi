...
import changeParam from "./modules/changeParam";
        ...
        dimensionSort.forEach(dimension => {
          ...
          targetInfoList.forEach(targetInfo => {
            ...
            if (element.propValue.viewId === targetViewId) { // 如果目标视图 和 当前循环组件id相等 则进行条件增减
              ...
              let operatorType = '';
              let dimensionValue ;
              //[eq, ne, gt, ge, lt, le, in, not in, like, not like, between]
              if('checkBox' == data.pluginType){//复选框
                operatorType = 'in'
                dimensionValue =dimension.value;
              }else if('dateSelect' == data.pluginType){//时间选择插件
                operatorType = 'between'
                dimensionValue =dimension.value;
              }else if('slider' == data.pluginType){//拖动条
                operatorType = 'le'
                dimensionValue =[dimension.value];
              }else{
                operatorType = 'eq'
              ...
              const condition = new Condition('', targetFieldId, operatorType, dimensionValue, [targetViewId])
              condition.sourceViewId = viewId
              let j = currentFilters.length
              while (j--) {
                const filter = currentFilters[j]

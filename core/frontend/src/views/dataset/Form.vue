...
export default {
  ...
  created() {
    ...
    const { datasetType, sceneId, id, editType, name: label, indicatorId, indicatorName, dataSource} = routeInfo
    this.datasetType = datasetType
    this.editType = editType
    if (id) {
      this.initTable(id)
    } else {
      let name = label || this.$t('commons.create') + this.$t(datasetTypeMap[datasetType]) + this.$t('auth.datasetAuth')
      if (indicatorId && (datasetType === 'sql' || datasetType === 'union')) {
        name = indicatorName;
      }
      this.table = {
        name,

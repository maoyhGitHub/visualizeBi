...
import {save} from '@/api/panel/indicators'
export default {
  name: 'AddDB',
  ...
  methods: {
    ...
    setActiveName({ name, datasourceId, enableCheck }) {
      ...
      this.activeTable = this.tableData.find((ele) => ele.name === name) || {}
      if (this.param.dataSource === 'indicators') {
        this.activeName = this.param.indicatorName
      }else {
    ...
    save() {
      ...
      const indicatorId = this.param.indicatorId
      ...
      post('/dataset/table/batchAdd', tables)
        .then((response) => {
          ...
          if (this.param.dataSource === 'indicators') {
            formData.id = indicatorId;
            formData.datasetTableId = response.data[0].id;

    ...
    <el-col>
      ...
      <el-col class="custom-tree-container de-tree">
        <div class="block">
          ...
          >
            <span
              v-if="data.modelInnerType === 'group' || data.modelInnerType === 'indicators' || data.modelInnerType === 'droup'"
              slot-scope="{ node, data }"
              class="custom-tree-node father"
            >
              <span style="display: flex; flex: 1; width: 0">
                <span v-if="data.modelInnerType === 'group' || data.modelInnerType === 'droup'">
                  <svg-icon icon-class="scene"/>
                </span>
                <span v-if="data.modelInnerType === 'indicators'">
                  <svg-icon icon-class="indicators"/>
                </span>
                <span
                  style="
                    margin-left: 6px;
...
export default {
  ...
  mounted() {
    this.init(true)
    // if(this.$route.query.datasetTableId){//指标库字段跳转过来情况
    //   this.$nextTick(() => {
    //     this.$refs.datasetTreeRef.setCurrentKey(this.$route.query.datasetTableId);
    //   })
    // }
  },
  beforeDestroy() {

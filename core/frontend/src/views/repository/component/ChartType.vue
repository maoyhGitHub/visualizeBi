<template>
  ...
  >
    <div
      ...
      :key="category"
    >
      <el-divider content-position="left" class="chart-type-divider">{{ $t(category) }}</el-divider>
        ...
        <div class="radio-row">
          ...
          >
            ...
            <el-radio
              ...
              class="radio-style"
            >
              <span :title="$t(item.title)" @click="getImgUrl(item)">
                <svg-icon
              ...
              <div>
                <el-dialog title="图表查看" :visible.sync="imgsVisible" width="45%" append-to-body>
                  <div style="  display: flex;justify-content: center;">
                    <el-image :src="imgUrl" fit="scale-down" lazy style="margin: 20px auto; width: 700px; height: 550px">
  ...
  methods: {
    ...
    formatTypes() {
      this.pluginTypes.forEach(item => {
        ...
        if (len === 0 || renderItem[category][len - 1].length === 12) {
    ...
    getImgUrl(data) {
      ...
      if (data.render === 'antv') {
        this.imgUrl = require('@/assets/viewComponent_images/antV/' + data.value + '.png')
      }else {
        this.imgUrl = require('@/assets/viewComponent_images/echarts/' + data.value + '.png')
      }
    }
  }
}

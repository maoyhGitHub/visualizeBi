...
<!--        <el-row style="height: 80px;margin-top:0px;margin-bottom:20px;overflow: hidden">
        ...
        <!--        <el-row style="height: 80px;margin-top:0px;margin-bottom:20px;overflow: hidden" v-if="activeName === 'upload'">
                  <el-col
                    :span="4"
                    style="padding-left: 10px"
                  >
        ...
        <el-row v-if="activeName === 'preview'">
          <el-col :span="6">
            <span>图片分类:</span>
          ...
          <el-col :span="18">
            <el-upload
              ...
              accept=".jpeg,.jpg,.png,.gif,.svg"
              list-type="picture">
              <el-button size="mini" style="margin-left: 15px;margin-top: 3px;" type="primary">上传图片</el-button>
            </el-upload>
            <el-input
              ...
              style="position: absolute;margin-left: 317px;margin-top: -32px;width: 180px;"
        ...
        <el-row v-if="showFlag" style="height: 300px; overflow: scroll">
          <el-table-column
            ...
            >
              <div>
                <img
                  ...
                  style="width: 120px;max-height: 100px; transform: scale(2); background-color: #f9f7f7;"
                  loading="lazy"
                />
              </div>
              <div slot="reference" class="imgDiv">
                <el-radio v-model="imgRadio" :label=item.fileName @change="onChangeImg(item.fileName)">
                  <template #default>
                    <div class="radio-option">
                      <div style="height: 100px; margin-left: 3px; position: absolute">
                        <img :src="windowUrl + item.fileName" style="width: 120px;max-height: 100px;" loading="lazy"/>
                      </div>
...
import {findAllResourceName, findResourceNameByCondition, uploadFileResult} from '@/api/staticResource/staticResource'
  ...
  watch: {
    activeName: {
      handler: function(newVal, oldVal) {
        this.clearOption(newVal, oldVal);
      }
    }
  },
  computed: {
    ...
    isFilterComponent() {
      return ['de-select','de-tree-list', 'de-select-grid', 'de-date', 'de-input-search', 'de-number-range', 'de-select-tree'].includes(this.curComponent.component)
    }
  },
  created() {
    //加载图片分类下拉框数据
    ...
    this.curComponent.commonBackground.backgroundType = 'outerImage'
    this.init()
    this.changeImageType(this.imageTypeId, '')
  },
  methods: {
    colorChange(val) {
      if (val === null) {
    ...
    //分类改变事件
    changeImageType(val, imgName){
      this.showFlag = true;
      //根据所选分类 加载图库图片
      try {
        findResourceNameByCondition({categoryId:val,name: imgName}).then((rsp) => {
    ...
    onChangeType() {
      if(this.activeName ==='frame'){
        this.curComponent.commonBackground.backgroundType = 'innerImage'
        this.showFlag = false;
      }else {
        this.curComponent.commonBackground.backgroundType = 'outerImage'
        this.showFlag = true;
    ...
    onChangeImg(fileName) {
      this.curComponent.commonBackground.outerImage = '/static-resource/' + fileName
    },
    ...
    clearOption(newVal, oldVal) {
      if (oldVal === 'preview') {
        this.imgRadio = null;
        this.curComponent.commonBackground.outerImage = ''
      }
    },
    search() {
      this.changeImageType(this.imageTypeId, this.imageNameText);
    }  }
}
</script>

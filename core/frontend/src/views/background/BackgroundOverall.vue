      ...
      >
        <el-row>
          ...
          <el-row v-if="showFlag" style="width: 252px!important;height: 275px; overflow-y: auto">
            <el-table-column prop="headimg" :show-overflow-tooltip="true" v-for="(item,index) in urlList" class="img">
              <div class="imgDiv">
                <el-radio
                  :label=item.fileName
                  v-model="imgRadio"
...
import {uploadFileResult, findAllResourceName, findResourceNameByCondition} from '@/api/staticResource/staticResource'
...
export default {
  ...
  methods: {
    ...
    onChangeImg(fileName) {
      this.commonBackground.backgroundType = "outerImage";
      this.themeChange('backgroundType')

      this.commonBackground.outerImage = '/static-resource/' + fileName

    ...
    >
      ...
      >
        <span :style="label_class"  >
          <div v-if="quotaColorFlag" :style="{'--quotaColor': quotaColor,'--quotaColor1':quotaColor1}">
          <span :style="label_content_class"  ref="labelText" :class="['gradient-text']"  @click="textClick" >
            {{ result }}
          </span>
          </div>
          <div v-else>
             <p :style="label_content_class" ref="labelText"  @click="textClick">
              {{ result }}
            </p>
          </div>
        </span>
        <span
          ...
          :style="label_space"
        >
          <p :style="label_class"  @click="textClick">
...
export default {
  ...
  methods: {
    ...
    textClick() {
      ...
      this.$refs['labelText'].offsetTop
      if (this.trackMenu.length < 2) { // 只有一个事件直接调用
        this.trackClick(this.trackMenu[0])
      } else { // 视图关联多个事件
        this.trackBarStyle.left = (this.$refs['labelText'].offsetLeft + this.$refs['labelText'].offsetWidth / 2) + 'px'
        this.trackBarStyle.top = (this.$refs['labelText'].offsetTop + this.$refs['labelText'].offsetHeight + 10) + 'px'
    ...
    initStyle() {
      if (this.chart.customAttr) {
        ...
        if (customAttr.color) {
          ...
          if(customAttr.color.quotaColor && customAttr.color.quotaColor1){//设置渐变色
            this.label_content_class.color = '';
            this.quotaColorFlag = true;
            this.quotaColor = customAttr.color.quotaColor;
            this.quotaColor1 = customAttr.color.quotaColor1;
          }else{
            this.quotaColorFlag = false;
          }
          this.colorThreshold(customAttr.color)
        ...
        if (senior.threshold && senior.threshold.labelThreshold && senior.threshold.labelThreshold.length > 0) {
          ...
          for (let i = 0; i < senior.threshold.labelThreshold.length; i++) {
            ...
            if (flag) {
              ...
              this.label_content_class.color = valueColor.quotaColor //
            }
          }
        } else {
          if(!valueColor.quotaColor || !valueColor.quotaColor1){ //如果没有选渐变色保持原来逻辑
            if(valueColor.quotaColor){
              this.label_content_class.color = valueColor.quotaColor
            }
            if(valueColor.quotaColor1){
              this.label_content_class.color = valueColor.quotaColor1
            }
          }
        }
      }

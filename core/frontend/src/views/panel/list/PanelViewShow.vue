      ...
      >
        ...
        <div style="height: 100%;">
          ...
          <el-col :span="12">
            ...
            <span
              v-if="hasDataPermission('manage',panelInfo.privileges)&&activeTab==='PanelList'&&!panelInfo.sourcePanelName && showStaticDataConfig"
...
<!--            <span
              v-if="showType !== 1 && showStaticDataConfig"
              style="float: right;margin-right: 10px"
            >
              <de-btn
                secondary
            ...
            <span
              v-if="(activeTab!=='panels_star' || (activeTab ==='panels_star' && panelInfo.status === 'publish')) && showStaticDataConfig"
              style="float: right;margin-right: 10px"
              class="de-tree"
            >
              <el-dropdown
                ...
                >
                  ...
                  <el-dropdown
                    ...
                    >
                      ...
                      >公共链接分享</el-dropdown-item>
                      <el-dropdown-item
                        icon="el-icon-s-cooperation"
                        @click.native="dataApprove"
                      >数据审批</el-dropdown-item>
                    </el-dropdown-menu>
                  </el-dropdown>
                </el-dropdown-menu>
              </el-dropdown>
...
import StaticDataLink from '@/views/link/generate/StaticDataLink.vue'
import StaticDataApprove from '@/views/panel/list/StaticDataApprove.vue'
...
export default {
  ...
  methods: {
    ...
    staticDataConfig() {
      if (this.fillingName === '填报数据') {
        this.fillingName = '填报完成'
        this.showStaticDataConfig = false
        this.$store.dispatch('changeParam/setShowStaticDataConfig', false)
      } else {
        this.fillingName = '填报数据'
        this.showStaticDataConfig = true
        this.$store.dispatch('changeParam/setShowStaticDataConfig', true)
      }
      this.refreshPanel()
      this.$emit('showStaticData', false)
    },
    LinkShare() {
      this.linkVisible = true
      this.linkResourceId = this.panelInfo.id
    },
    removeLink() {

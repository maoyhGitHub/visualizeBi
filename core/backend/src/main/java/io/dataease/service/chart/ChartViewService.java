...
import io.dataease.plugins.common.base.mapper.DatasetStaticDataMapper;
...
public class ChartViewService {
    ...
    @Resource
    private DatasetStaticDataMapper datasetStaticDataMapper;
    ...
    public ChartViewDTO calcData(ChartViewDTO view, ChartExtRequest chartExtRequest, boolean cache) throws Exception {
        ...
        //处理过滤参数为空的数据
        List<ChartExtFilterRequest> newList = chartExtRequest.getLinkageFilters().stream()
                .filter(chartExtFilterRequest -> !"".equals(chartExtFilterRequest.getValue().get(0)))
                .collect(Collectors.toList());
        chartExtRequest.setLinkageFilters(newList);
        ChartCustomParam chartCustomParam = new ChartCustomParam();
        if (ObjectUtils.isEmpty(view)) {
            throw new RuntimeException(Translator.get("i18n_chart_delete"));
        }
        ...
        List<String> dataeaseNames = columnPermissionFields.stream().map(DatasetTableField::getDataeaseName).collect(Collectors.toList());
        dataeaseNames.add("*");
        fieldCustomFilter = fieldCustomFilter.stream().filter(item -> chartViewFieldNameList.contains(item.getDataeaseName()) || (!desensitizationList.containsKey(item.getDataeaseName()) && dataeaseNames.contains(item.getDataeaseName()))).collect(Collectors.toList());
        extStack = extStack.stream().filter(item -> chartViewFieldNameList.contains(item.getDataeaseName()) || (!desensitizationList.containsKey(item.getDataeaseName()) && dataeaseNames.contains(item.getDataeaseName()))).collect(Collectors.toList());
        extBubble = extBubble.stream().filter(item -> chartViewFieldNameList.contains(item.getDataeaseName()) || (!desensitizationList.containsKey(item.getDataeaseName()) && dataeaseNames.contains(item.getDataeaseName()))).collect(Collectors.toList());
        drill = drill.stream().filter(item -> chartViewFieldNameList.contains(item.getDataeaseName()) || (!desensitizationList.containsKey(item.getDataeaseName()) && dataeaseNames.contains(item.getDataeaseName()))).collect(Collectors.toList());
        ...
        chartCustomParam.setXAxis(xAxis);
        chartCustomParam.setYAxis(yAxis);
        if (CollectionUtils.isEmpty(xAxis) && CollectionUtils.isEmpty(yAxis)) {
        ...
        switch (view.getType()) {
            case "label":
                xAxis = xAxis.stream().filter(item -> chartViewFieldNameList.contains(item.getDataeaseName()) || (!desensitizationList.containsKey(item.getDataeaseName()) && dataeaseNames.contains(item.getDataeaseName()))).collect(Collectors.toList());
                yAxis = new ArrayList<>();
                if (CollectionUtils.isEmpty(xAxis)) {
                    return emptyChartViewDTO(view);
                }
            ...
            case "liquid":
                xAxis = new ArrayList<>();
                yAxis = yAxis.stream().filter(item -> chartViewFieldNameList.contains(item.getDataeaseName()) || (!desensitizationList.containsKey(item.getDataeaseName()) && dataeaseNames.contains(item.getDataeaseName()))).collect(Collectors.toList());
                if (CollectionUtils.isEmpty(yAxis)) {
                    return emptyChartViewDTO(view);
                }
                break;
            ...
            case "flow-map":
                xAxis = xAxis.stream().filter(item -> chartViewFieldNameList.contains(item.getDataeaseName()) || (!desensitizationList.containsKey(item.getDataeaseName()) && dataeaseNames.contains(item.getDataeaseName()))).collect(Collectors.toList());
                yAxis = yAxis.stream().filter(item -> chartViewFieldNameList.contains(item.getDataeaseName()) || (!desensitizationList.containsKey(item.getDataeaseName()) && dataeaseNames.contains(item.getDataeaseName()))).collect(Collectors.toList());
                xAxisBase = xAxisBase.stream().filter(item -> chartViewFieldNameList.contains(item.getDataeaseName()) || (!desensitizationList.containsKey(item.getDataeaseName()) && dataeaseNames.contains(item.getDataeaseName()))).collect(Collectors.toList());
                xAxisExt = xAxisExt.stream().filter(item -> chartViewFieldNameList.contains(item.getDataeaseName()) || (!desensitizationList.containsKey(item.getDataeaseName()) && dataeaseNames.contains(item.getDataeaseName()))).collect(Collectors.toList());
                break;
            default:
                xAxis = xAxis.stream().filter(item -> chartViewFieldNameList.contains(item.getDataeaseName()) || (!desensitizationList.containsKey(item.getDataeaseName()) && dataeaseNames.contains(item.getDataeaseName()))).collect(Collectors.toList());
                yAxis = yAxis.stream().filter(item -> chartViewFieldNameList.contains(item.getDataeaseName()) || (!desensitizationList.containsKey(item.getDataeaseName()) && dataeaseNames.contains(item.getDataeaseName()))).collect(Collectors.toList());
                ...
                if (StringUtils.isNotEmpty(fieldId)) {
                    ...
                    if (request.getIsTree()) {
                        ...
                        for (String fId : fieldIds) {
                            ...
                            if (!desensitizationList.containsKey(datasetTableField.getDataeaseName()) && dataeaseNames.contains(datasetTableField.getDataeaseName())) {
...
//              堆叠
        ...
        // 如果是插件视图 走插件内部的逻辑
        if (ObjectUtils.isNotEmpty(view.getIsPlugin()) && view.getIsPlugin() && !"view-dev-tool".equals(view.getType())) {
            ...
            if(false){
                chartCustomParam.setFieldCustomFilter(fieldCustomFilter);
                chartCustomParam.setRowPermissionsTree(rowPermissionsTree);
                chartCustomParam.setExtFilterList(extFilterList);
                //数据转换处理
                this.convertBindDatasetData(table,view,chartExtRequest, mapData, chartCustomParam);
            }
            //如果指标与维度都选了，走源码内部组件Sql，反之走源码非组件sql
            if(CollectionUtils.isNotEmpty(xAxis) && CollectionUtils.isNotEmpty(yAxis)){
                fieldMap.put("extStack", extStack);
                fieldMap.put("extBubble", extBubble);
                fieldMap.put("xAxis", xAxis);
                fieldMap.put("yAxis", yAxis);
                ...
                if (StringUtils.isBlank(sql)) {
                    return emptyChartViewDTO(view, mapData);
                }
                datasourceRequest.setQuery(sql);
                data = datasourceProvider.getData(datasourceRequest);
                ...
                return uniteViewResultConvert(datasourceRequest.getQuery(), mapChart, mapTableNormal, view, isDrill, drillFilters, dynamicAssistFields, assistData, mapData);
                // 如果是插件到此结束
            }
        }
        ...
        if (table.getMode() == 0) {// 直连
            ...
            DataSetStaticData  staticData= datasetStaticDataMapper.selectByTableId(table.getId(),view.getId());
            boolean dataFlag = true;
            if(Objects.nonNull(staticData)){
                if(staticData.getStatus() ==2){
                    JSONArray jsonArray = JSON.parseArray(staticData.getExcelNewData());
                    data = jsonArray.toJavaList(String[].class);
                    dataFlag = false;
                }
            }
            if(dataFlag){
    ...
    private ChartViewDTO emptyChartViewDTO(ChartViewDTO view,Map<String, Object> map) {
        Map<String, Object> dataMap = new HashMap<>();
        ChartViewDTO dto = new ChartViewDTO();
        BeanUtils.copyBean(dto, view);
        dto.setTableAllData(map);
        dataMap.put("fields",map.get("fields"));
        if(null == dto.getData() ){ //data 为空情况 将allData数据赋值给data
            dto.setData(dataMap);
        }
        return dto;
    }
    ...
    private ChartViewDTO uniteViewResultConvert(String sql, Map<String, Object> chartData, Map<String, Object> tableData, ChartViewDTO view, Boolean isDrill, List<ChartExtFilterRequest> drillFilters, List<ChartSeniorAssistDTO> dynamicAssistFields, List<String[]> assistData, Map<String, Object> tableAllData) {
        Map<String, Object> map = new HashMap<>();
        map.putAll(chartData);
        map.putAll(tableData);
        ...
        dto.setDrill(isDrill);
        dto.setDrillFilters(drillFilters);
        dto.setTableAllData(tableAllData);
        return dto;
    }


    private void convertBindDatasetData(DataSetTableDTO table,ChartViewDTO view,ChartExtRequest chartExtRequest, Map<String,Object> mapData, ChartCustomParam  chartCustomParam) throws Exception {
        DataSetTableRequest dataSetTableRequest = new DataSetTableRequest();
        Datasource ds = table.getMode() == 0 ? datasourceService.get(table.getDataSourceId()) : engineService.getDeEngine();
        dataSetTableRequest.setRow(view.getResultCount().toString());
        dataSetTableRequest.setId(view.getTableId());
        dataSetTableRequest.setDataSourceId(ds.getId());
        dataSetTableRequest.setInfo(table.getInfo());
        String querySql = null;

        List<ChartViewFieldDTO> xAxis = chartCustomParam.getXAxis();
        List<ChartViewFieldDTO> yAxis = chartCustomParam.getYAxis();
        QueryProvider qp = ProviderFactory.getQueryProvider(ds.getType());
        DataTableInfoDTO dataTableInfoDTO = gson.fromJson(table.getInfo(), DataTableInfoDTO.class);
        ...
        Map<String, Object> tableAllData = dataSetTableService.getPreviewData(dataSetTableRequest,1,view.getResultCount(),null,null);
        List<DatasetTableField> fields = (ArrayList)tableAllData.get("fields");
        if(table.getMode() == 0){//直连情况
            //sql级sql查询
            if (StringUtils.equalsIgnoreCase(table.getType(), DatasetType.SQL.name())) {
                String sql = dataTableInfoDTO.isBase64Encryption() ? new String(java.util.Base64.getDecoder().decode(dataTableInfoDTO.getSql())) : dataTableInfoDTO.getSql();
                if("tasksCommonCode".equals(view.getType())){ // 任务通用代码
                    querySql = qp.getSQLAsTmp(sql, xAxis, yAxis, chartCustomParam.getFieldCustomFilter(), chartCustomParam.getRowPermissionsTree(),chartCustomParam.getExtFilterList(), view);
                } else if("tasksPanel".equals(view.getType())){// 多数据集情况 (西飞自定义组件使用)
                    querySql = qp.getSQLWithPage(false, sql, xAxis, null, null, chartExtRequest.getLinkageFilters(), ds, view, new PageInfo());
                }else{//其他自定义组件 累计
                    querySql = qp.getSQLSummaryAsTmp(sql, yAxis, null, null, chartExtRequest.getLinkageFilters(), view);//拼接sql
                }

                datasourceRequest.setQuery(querySql);//查询结果
                List<String[]> filterData =  datasourceProvider.getData(datasourceRequest);


                List<ChartViewFieldDTO> chartViewFieldDTOS = new ArrayList<>();
                fields.forEach(item->{
                    ChartViewFieldDTO chartViewFieldDTO = new ChartViewFieldDTO();
                    chartViewFieldDTO.setId(item.getId());
                    chartViewFieldDTO.setDataeaseName(item.getDataeaseName());
                    chartViewFieldDTO.setDeType(item.getDeType());
                    chartViewFieldDTOS.add(chartViewFieldDTO);
                });
                if(xAxis.size() == 0 && yAxis.size() == 0){//没有拖拽维度和指标的情况
                    xAxis = chartViewFieldDTOS;
                }
                if(xAxis.size() > 0){
                    if(ObjectUtils.allNull(xAxis.get(0).getDataeaseName(),xAxis.get(0).getId(),xAxis.get(0).getDeType())){
                        DEException.throwException("维度缺少属性,请检查数据正确性！");
                    }
                }
                if(yAxis.size() > 0 ){
                    if(ObjectUtils.allNull(yAxis.get(0).getId(),yAxis.get(0).getDataeaseName(),yAxis.get(0).getDeType())){
                        DEException.throwException("指标缺少属性,请检查数据正确性！");
                    }
                }
                //转换查询结果
                Map<String, Object> filterDataMap  = ChartDataBuild.transTableNormal(xAxis, yAxis, view, filterData, new ArrayList<>(), new HashMap<>());
                filterDataMap.put("filterParam" ,chartExtRequest.getLinkageFilters());
                mapData.put("filterData",filterDataMap);

            } else  if (StringUtils.equalsIgnoreCase(table.getType(), DatasetType.DB.name())) { //DB数据集查询
                datasourceRequest.setTable(dataTableInfoDTO.getTable());
                if ("tasksCommonCode".equals(view.getType())) {
                    querySql = qp.getSQL(dataTableInfoDTO.getTable(), xAxis, yAxis, null, null, chartExtRequest.getLinkageFilters(), ds, view);
                }else if("tasksPanel".equals(view.getType())){// 多数据集情况 (西飞自定义组件使用)
                    querySql = qp.getSQLWithPage(true, dataTableInfoDTO.getTable(), xAxis, null, null, chartExtRequest.getLinkageFilters(), ds, view, new PageInfo());
                } else {
                    querySql = qp.getSQLSummary(dataTableInfoDTO.getTable(), yAxis, null, null, chartExtRequest.getLinkageFilters(), view, ds);
        ...
        if (ObjectUtils.isNotEmpty(chartExtRequest.getLinkageFilters())) {
            ...
            List<ChartExtFilterRequest> chartExtRequestLinkageFilters = chartExtRequest.getLinkageFilters();
            chartExtRequestLinkageFilters.forEach(chartExtFilterRequest -> {
                DatasetTableField datasetTableField = dataSetTableFieldsService.get(chartExtFilterRequest.getFieldId());
                whereMap.put(datasetTableField.getDataeaseName(),chartExtFilterRequest.getValue().get(0));
            });
            List<Map<String,String>> dataList = (List<Map<String,String>>) tableAllData.get("data");
            List<Map<String,String>> linkageFiltersData = new ArrayList<>();
            dataList.forEach(data->{
                AtomicInteger count = new AtomicInteger();
                data.forEach((k,v)->{
                    if(StringUtils.isNotEmpty(whereMap.get(k))) {
                        if(StringUtils.equals(whereMap.get(k),v)){
                            count.getAndIncrement();
                        }
                    }
                });
                if(whereMap.size() ==  count.get()){
                    linkageFiltersData.add(data);
                }
            });
            mapData.put("linkageFiltersData",linkageFiltersData);
            mapData.put("linkageFiltersParam",whereMap);
        }
        mapData.put("fields",fields);//树列表使用
        mapData.put("allData",tableAllData);
    }
}

...
import io.dataease.ext.PanelIndicatorsMapper;
...
public class DataSetTableService {
    ...
    public void alter(DataSetTableRequest request) {
        ...
        PanelIndicatorsDTO panelIndicatorsDTO = new PanelIndicatorsDTO();
        panelIndicatorsDTO.setName(request.getName());
        panelIndicatorsDTO.setDatasetTableId(request.getId());
        panelIndicatorsMapper.updateIndicatorsByDatasetId(panelIndicatorsDTO);
        ...
        if (CollectionUtils.isNotEmpty(data)) {
            jsonArray = data.stream().map(ele -> {
                ...
                for (int i = 0; i < ele.length; i++) {
                    if (desensitizationList.containsKey(fieldArray[i])) {
    ...
    public void updateDatasetTableStatus() {
        if (isUpdatingDatasetTableStatus) {
            return;
        } else {
            isUpdatingDatasetTableStatus = true;
        }
        ...
        try {
            ...
            isUpdatingDatasetTableStatus = false;
        }
    }
    ...
    public DataSetStaticData saveTempStaticData(DataSetStaticData param) {
        DataSetStaticData staticData = datasetStaticDataMapper.selectByTableId(param.getTableId(),param.getViewId());
        if(Objects.nonNull(staticData)){
            return staticData;
        }else {
            datasetStaticDataMapper.insert(param);
        }
        return datasetStaticDataMapper.selectByTableId(param.getTableId(),param.getViewId());
    }
    public void updateTempStaticData(DataSetStaticData param) {
        datasetStaticDataMapper.updateByTableId(param);
    }

    public List<Map<String, Object>> importCustomExcel(MultipartFile file, DataSetStaticData param) {
        List<Map<String, Object>> returnSheetDataList = new ArrayList<>();
        String filename = file.getOriginalFilename();
        try {
            assert filename != null;
            List<ExcelSheetData> excelSheetDataList = parseExcel(filename, file.getInputStream(), true);
            List<DatasetTableField> fields = dataSetTableFieldsService.getFieldsByTableId(param.getTableId());
            Map<String, List<DatasetTableField>> fieldMap = fields.stream().collect(Collectors.groupingBy(DatasetTableField::getOriginName,Collectors.toList()));
            List<Map<String,Object>> dataJsonArray = excelSheetDataList.get(0).getJsonArray();
            for( Map<String, Object> mapList : dataJsonArray ) {
                Map<String,Object> data = new HashMap<>();
                for (Map.Entry<String,Object> entry : mapList.entrySet()) {
                    List<DatasetTableField> fieldList = fieldMap.get(entry.getKey());
                    data.put(fieldList.get(0).getDataeaseName(),entry.getValue());
                }
                returnSheetDataList.add(data);
            }
        }catch (Exception e){
            DataEaseException.throwException("excel解析异常");
        }
        return returnSheetDataList;
    }

    public List<DataSetStaticData> staticDataList(DataSetStaticData param) {
        return datasetStaticDataMapper.staticDataList(param);
    }
}